  ...
  return (
    <div className="min-h-screen bg-[#F7F7F7] text-[#111] max-w-[430px] mx-auto">
      ...
      <div className="px-5 pt-14 pb-6 animate-fadeInUp">
        ...
        <h2 className="text-3xl font-black tracking-tight text-[#111]">部位を選ぶ</h2>
        <p className="text-[#BBBBBB] text-xs mt-1.5">トレーニングする部位を選択</p>
      </div>
        ...
        {PARTS.map((key, idx) => {
          ...
          return (
            <button
              key={key}
              onClick={() => select(key)}
              className="relative rounded-2xl overflow-hidden active:scale-[0.96] transition-transform shadow-lg"
              style={{ aspectRatio: '3/4', animationDelay: `${idx * 0.04}s` }}
            >
              {/* Photo background */}
              <div
              ...
              {/* Recommended badge */}
              {isRec && (
                <div className="absolute top-3 right-3 bg-[#00DD77] rounded-full px-2 py-0.5">
